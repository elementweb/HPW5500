...
class HPW5500WebSocketServer {
    ...
    public: void forceDisconnect(uint8_t socket);
    public: void setBestEffort(bool enabled);
    ...
    private: struct ConnectionState {
        ...
        uint16_t frag_len = 0;
        uint16_t tcp_buf_len = 0;
        uint8_t frag_buffer[2048];
        uint8_t tcp_buf[2064];
        char path[48];
        char cookie[64];
    };
    ...
    private: uint16_t handleFrame(uint8_t socket, const uint8_t *data, uint16_t length);
    ...
    private: bool best_effort = false;
